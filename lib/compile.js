...
function compilePostCollection (dirs) {
  return __obtainContentDirs(dirs)
...
function __obtainContentDirs (config) {
  return qfs.list(config.contentDir)
    .then(function (dirs) {
      return _.map(dirs, dir => ({ dir: path.join(config.contentDir, dir) }));
    });
}
 ...
 */
function __assignFrontmatter (posts) {
  return Q.all(_.map(posts, function (post) {
    return qfs.read(path.join(post.dir, "frontmatter.json"))
      .then(function (fm) {
 ...
 */
function __assignTargetDir (posts) {
  return Q.resolve(_.map(posts, post => {
